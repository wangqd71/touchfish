...
# 六十四卦系统 (阴阳属性)
# ============================================================
# 装备槽位 → 爻位 (从下到上: 饰品→鞋子→胸甲→头盔→副手→主手)
SLOT_LINE_ORDER = [SLOT_ACCESSORY, SLOT_BOOTS, SLOT_CHEST, SLOT_HELMET, SLOT_OFF_HAND, SLOT_MAIN_HAND]

# 阴阳显示
YIN_DISPLAY = "—     —"   # 阴 (broken: 7 chars)
YANG_DISPLAY = "———"      # 阳 (solid: 3 chars)
YY_NONE = -1
YY_YIN = 0
YY_YANG = 1

# 64卦数据: key = 6位二进制(从下到上), value = (卦名, Unicode符号, 加成类型, 加成数值)
HEXAGRAMS = {
    # 乾宫 (上☰)
    "111111": ("乾为天",   "䷀", "dmg_mult", 0.10),
    "111011": ("天火同人", "䷌", "crit_rate", 0.05),
    "111101": ("天山遁",   "䷠", "dodge_rate", 0.05),
    "111001": ("天风姤",   "䷫", "atk_mult", 0.08),
    "111110": ("天泽履",   "䷉", "def_mult", 0.08),
    "111010": ("天水讼",   "䷅", "crit_dmg", 0.15),
    "111100": ("天地否",   "䷋", "hp_mult", 0.08),
    "111000": ("天雷无妄", "䷘", "exp_mult", 0.10),
    # 兑宫 (上☱)
    "011111": ("泽天夬",   "䷪", "atk_mult", 0.08),
    "011011": ("兑为泽",   "䷱", "gold_mult", 0.15),
    "011101": ("泽山咸",   "䷞", "crit_rate", 0.04),
    "011001": ("泽风大过", "䷛", "dmg_mult", 0.08),
    "011110": ("泽雷随",   "䷐", "speed_mult", 0.08),
    "011010": ("泽水困",   "䷮", "def_mult", 0.10),
    "011100": ("泽地萃",   "䷬", "hp_mult", 0.06),
    "011000": ("泽火革",   "䷰", "crit_dmg", 0.12),
    # 离宫 (上☲)
    "101111": ("火天大有", "䷍", "dmg_mult", 0.08),
    "101011": ("火泽睽",   "䷥", "crit_rate", 0.04),
    "101101": ("火山旅",   "䷷", "dodge_rate", 0.06),
    "101001": ("火风鼎",   "䷱", "atk_mult", 0.10),
    "101110": ("火雷噬嗑", "䷔", "crit_dmg", 0.12),
    "101010": ("离为火",   "䷝", "dmg_mult", 0.06),
    "101100": ("火地晋",   "䷢", "exp_mult", 0.08),
    "101000": ("天火同人", "䷌", "gold_mult", 0.12),
    # 震宫 (上☳)
    "001111": ("雷天大壮", "䷡", "atk_mult", 0.10),
    "001011": ("雷泽归妹", "䷵", "crit_rate", 0.03),
    "001101": ("雷山小过", "䷽", "dodge_rate", 0.08),
    "001001": ("震为雷",   "䷲", "crit_dmg", 0.15),
    "001110": ("雷风恒",   "䷟", "def_mult", 0.06),
    "001010": ("雷水解",   "䷧", "hp_mult", 0.06),
    "001100": ("雷地豫",   "䷏", "speed_mult", 0.06),
    "001000": ("雷火丰",   "䷶", "dmg_mult", 0.06),
    # 巽宫 (上☴)
    "110111": ("风天小畜", "䷈", "def_mult", 0.08),
    "110011": ("风泽中孚", "䷼", "hp_mult", 0.06),
    "110101": ("风山渐",   "䷴", "dodge_rate", 0.06),
    "110001": ("巽为风",   "䷸", "speed_mult", 0.10),
    "110110": ("风雷益",   "䷩", "atk_mult", 0.06),
    "110010": ("风水涣",   "䷺", "crit_rate", 0.04),
    "110100": ("风地观",   "䷓", "exp_mult", 0.08),
    "110000": ("风火家人", "䷤", "gold_mult", 0.12),
    # 坎宫 (上☵)
    "010111": ("水天需",   "䷄", "hp_mult", 0.10),
    "010011": ("水泽节",   "䷻", "def_mult", 0.08),
    "010101": ("水山蹇",   "䷦", "dodge_rate", 0.04),
    "010001": ("水风井",   "䷯", "atk_mult", 0.06),
    "010110": ("水雷屯",   "䷂", "crit_rate", 0.03),
    "010010": ("坎为水",   "䷜", "hp_mult", 0.08),
    "010100": ("水地比",   "䷇", "gold_mult", 0.10),
    "010000": ("水火既济", "䷾", "dmg_mult", 0.06),
    # 艮宫 (上☶)
    "100111": ("山天大畜", "䷙", "def_mult", 0.12),
    "100011": ("山泽损",   "䷨", "atk_mult", 0.06),
    "100101": ("艮为山",   "䷳", "dodge_rate", 0.10),
    "100001": ("山风蛊",   "䷑", "crit_dmg", 0.10),
    "100110": ("山雷颐",   "䷚", "hp_mult", 0.06),
    "100010": ("山水蒙",   "䷃", "exp_mult", 0.10),
    "100100": ("山地剥",   "䷖", "gold_mult", 0.08),
    "100000": ("山火贲",   "䷕", "dmg_mult", 0.06),
    # 坤宫 (上☷)
    "000111": ("地天泰",   "䷊", "hp_mult", 0.10),
    "000011": ("地泽临",   "䷒", "atk_mult", 0.06),
    "000101": ("地山谦",   "䷎", "def_mult", 0.10),
    "000001": ("地风升",   "䷭", "exp_mult", 0.12),
    "000110": ("地雷复",   "䷗", "crit_rate", 0.04),
    "000010": ("地水师",   "䷆", "dmg_mult", 0.08),
    "000100": ("坤为地",   "䷁", "gold_mult", 0.20),
    "000000": ("地火明夷", "䷣", "dodge_rate", 0.06),
}
